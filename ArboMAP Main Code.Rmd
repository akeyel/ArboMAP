...
modeltype = 'thinplate', use.cluster = 0){
#**# FIX THIS WHEN FIXING THE DOCUMENTATION
  # model type is either fixed df cubic regression spline or a thin-plate spline
#modeltype <- "fixeddfcr" #thinplate
  #**# ADD THIS TO DOCUMENTATION
  #**# Add use.cluster to documentation. If 1, it will attempt to use parallel processing to speed up the human.data.2 calculations. Otherwise it will not. The original uses the cluster as the only option. Here the default was changed to not use it, as it caused problems on my machine.
...
weekinquestionSatstr <- strftime(weekinquestionSat, '%A')
  ...
  #**# LOOKS LIKE THIS IS ALREADY AN INPUT. CUT FOLLOWING WALK-THROUGH
...
weathersummaryfile <- "weather data summary file.csv"
  ...
  # Check that model type was properly defined
  if (!(modeltype %in% c("fixeddfcr", "thinplate"))) {
    stop(sprintf("ModelType must be 'fixeddfcr' or 'thinplate'. You entered %s", modeltype))
    }
  
  if (RUN.NOW == 0){
    results = call.ArboMAP(humandatafile, mosqfile, districtshapefile, stratafile, weatherpathstr, weathersummaryfile, maxobservedhumandate, weekinquestion, var1name, var2name, compyear1, compyear2, results.path, graphicoutputdir, fullcasematoutputdir, mosqmatoutputdir, weekinquestionSun, weekinquestionSat, weekinquestionSunstr, weekinquestionSatstr, maxmosqyear, maxdesiredhumandate, maxdesiredhumandatestr, laglen, dlagdeg, original.metrics, modeltype, use.cluster)
  }
  ...
  inputs = list(graphicoutputdir, fullcasematoutputdir, mosqmatoutputdir, weekinquestionSun, weekinquestionSat, weekinquestionSunstr, weekinquestionSatstr, maxmosqyear, maxdesiredhumandate, maxdesiredhumandatestr, laglen, dlagdeg, modeltype, use.cluster)
...
call.ArboMAP = function(humandatafile, mosqfile, districtshapefile, stratafile, weatherpathstr, weathersummaryfile, maxobservedhumandate, weekinquestion, var1name, var2name, compyear1, compyear2, results.path, graphicoutputdir, fullcasematoutputdir, mosqmatoutputdir, weekinquestionSun, weekinquestionSat, weekinquestionSunstr, weekinquestionSatstr, maxmosqyear, maxdesiredhumandate, maxdesiredhumandatestr, laglen, dlagdeg, original.metrics, modeltype, use.cluster){
  ...
  phd2 = process.human.data2(dailyextr, fullcasemat, randeffs, var1name, var2name, strata, maxdesiredhumandate, minmosqyear, maxmosqyear, laglen, graphicoutputdir, compyear1, compyear2, weekinquestionSun, use.cluster)
    tempdf = phd2[[1]]
    tempdf2 = phd2[[2]]
    fullcasemat = phd2[[3]]
    firstreg = phd2[[4]]
  ...
  # Create the distributed lag plots
  dist.lag.out = dist.lag.setup(firstreg)
  lag1df = dist.lag.out[[1]]
  lag2df = dist.lag.out[[2]]
  
  distributed.lag.plots(lag1df, lag2df, var1name, var2name)
...
#**# Getting warning about uninitalized columns for var1, meanvar1, var2, meanvar2. Unclear where these columns are being called, or why this is an issue.
...
process.human.data2 = function(dailyextr, fullcasemat, randeffs, var1name, var2name, strata, maxdesiredhumandate, minmosqyear, maxmosqyear, laglen, graphicoutputdir, compyear1, compyear2, weekinquestionSun, use.cluster){
...
head(mean1data)
  ...
  if (modeltype == "fixeddfcr") {
    
    myformula <- paste("s(lag, by=",
                       names(fullcasemat)[grep(x=names(fullcasemat),
                                               pattern="dl_",
                                               fixed=TRUE)],
                       ", bs='cr', fx=TRUE, k=6)",sep="", collapse=" + ")
    myformula <- paste("anycases ~ 0 + MIRsummarystat + district", myformula, sep=" + ")
    myform <- as.formula(myformula)
    
  }
  
  # Allow option to use cluster or not
  if (use.cluster == 1){
...
#**# OLD VERSION (Retained to make sure the merge proceeded correctly)
  # show predictions
  ...
  #**# Where does tempfull actually get used? I do not see this variable called anywhere downstream
...
plot(thisplot)
  ...
  return(list(tempdf, tempdf2, fullcasemat, firstreg))
}
if (RUN.NOW == 1){
  phd2 = process.human.data2(dailyextr, fullcasemat, randeffs, var1name, var2name, strata, maxdesiredhumandate, minmosqyear, maxmosqyear, laglen, graphicoutputdir, compyear1, compyear2, weekinquestionSun, use.cluster)
  tempdf = phd2[[1]]
...
```{r echo=FALSE}
#**# THIS NEEDS REVIEW - LOOKS LIKE TWO OF THE OUTPUTS ARE NO LONGER PRODUCED HERE - NEED TO TALK ABOUT THIS IF FURTHER EDITS DO NOT MAKE THE EXPLANATION APPARENT. #**# NEEDS ATTENTION THIS WILL BREAK THE CODE.
write.case.matrix.and.calc.cases = function(fullcasemat, fullcasematoutputdir, maxmosqyear){
  ...
  #**# Fix this on a walk-through
  #**# casesthisyear looks like it comes out of preds
  #**# multiplier can probably be dropped
...
Below, we visualize the distributed lags by stratum. If a line is above (below) zero at some lag, this means risk today depends positively (negatively) on the environmental data at that date; e.g. precipitation a week ago (lag = 7) might raise risk today.

```{r distributedlags, include=FALSE, echo=FALSE}
dist.lag.setup = function(firstreg){
  ...
  for (i in 1:length(mygam)) {
    ...
    if (grepl(x=thisplot$ylab,
      ...
      newdf <- data.frame(fit=thisplot$fit,
                          x  =thisplot$`x`,
                          stratum=thisplot$ylab)
      lag1df <- bind_rows(lag1df, newdf)
    ...
    if (grepl(x=thisplot$ylab,
      ...
      newdf <- data.frame(fit=thisplot$fit,
                          x  =thisplot$`x`,
                          stratum=thisplot$ylab)
      lag2df <- bind_rows(lag2df, newdf)
      
    }
    
  }
  return(list(lag1df, lag2df))
}
...
distributed.lag.plots = function(lag1df, lag2df, var1name, var2name){
ggplot(lag1df) + geom_line(aes(x=x, y=fit, group=stratum, color=stratum)) +
  ggtitle(paste("distributed lags on ", var1name, sep="")) + xlab("lag")
ggplot(lag2df) + geom_line(aes(x=x, y=fit, group=stratum, color=stratum)) +
...
ggplot() + geom_line(data=termsum, aes(x=doy, y=var1inf, group=year), alpha=0.5) +
...
ggplot() + geom_line(data=termsum, aes(x=doy, y=var2inf, group=year), alpha=0.5) +
