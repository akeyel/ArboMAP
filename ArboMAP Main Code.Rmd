...
ArboMAP = function(humandatafile, mosqfile, districtshapefile, stratafile, weatherpathstr, weathersummaryfile, maxobservedhumandate, weekinquestion, var1name, var2name, compyear1, compyear2, results.path){
...
if (RUN.NOW == 1){
  out = ArboMAP(humandatafile, mosqfile, districtshapefile, stratafile, weatherpathstr, weathersummaryfile, maxobservedhumandate, weekinquestion, var1name, var2name, compyear1, compyear2, results.path)
  inputs = out[[2]] 
...
call.ArboMAP = function(humandatafile, mosqfile, districtshapefile, stratafile, weatherpathstr, weathersummaryfile, maxobservedhumandate, weekinquestion, var1name, var2name, compyear1, compyear2, results.path, graphicoutputdir, fullcasematoutputdir, mosqmatoutputdir, weekinquestionSun, weekinquestionSat, weekinquestionSunstr, weekinquestionSatstr, maxmosqyear, maxdesiredhumandate, maxdesiredhumandatestr, laglen, dlagdeg){
  ...
  dailyextr = plot.weather.data(weather, graphicoutputdir)
  ...
  mdp2 = mosquito.data.process2(wnv, stratafile, mosqmatoutputdir, graphicoutputdir, minmosqyear, maxmosqyear)
    wnv = mdp2[[1]]
    infectglm = mdp2[[2]]
    randeffs = mdp2[[3]]
    strata = mdp2[[4]]
  ...
  mosq.by.month(wnv, infectglm, minmosqyear, maxmosqyear, compyear1, compyear2, graphicoutputdir)
  ...
  thisplot2 = proportion.positive.plot(tempdf,tempdf2, maxmosqyear, compyear1, compyear2, graphicoutputdir)
  ...
  make.raw.risk.map(districtshapefile, fullcasemat, graphicoutputdir)

  # Create the relative risk map
  make.riskcalcs(fullcasemat, weekinquestion, projected_districts.df, graphicoutputdir, thisplot2)
  
  Districts.With.Cases.Focal.Week = round(66*tempdf2$est,1)
  # compile the main results into the results object
  results = list(DistrictsWithCasesInFocalWeek = Districts.With.Cases.Focal.Week, AnnualPositiveDistrictWeeks = positivesthisyear, Multiplier = multiplier, HumanCasesForYear = casesthisyear)
...
plot.weather.data = function(weather, graphicoutputdir){
  ...
  save1 = ggsave(paste(graphicoutputdir,
             "var1.png",
             sep=""), plot1)
  save2 = ggsave(paste(graphicoutputdir,
             "var2.png",
             sep=""), plot2)
...
mosquito.data.process2 = function(wnv, stratafile, mosqmatoutputdir, graphicoutputdir, minmosqyear, maxmosqyear){
...
mosq.by.month = function(wnv, infectglm, minmosqyear, maxmosqyear, compyear1, compyear2, graphicoutputdir){
...
proportion.positive.plot = function(tempdf, tempdf2, maxmosqyear, compyear1, compyear2, graphicoutputdir){
...
if (!exists("positivesthisyear")){
  positivesthisyear = multiplier = casesthisyear = -9999
}
...
make.raw.risk.map = function(districtshapefile, fullcasemat, graphicoutputdir){
  district_shapes <- readShapePoly(districtshapefile)
...
make.riskcalcs = function(fullcasemat, weekinquestion, projected_districts.df, graphicoutputdir, thisplot2){
